...
class Sketch:
    ...
    def __init__(self, label_end_path=None,draw_end_path=None,image_path=None,caption_coco=None,useless=True,color=True,scene=""):
        self.resolution = []
        self.image_name = ""
...
def get_train_test():
    path_image = "/home/zzm/datasets/coco2017/train2017"
    path_caption = "/home/zzm/datasets/coco2017/annotations/captions_train2017.json"

    save_path = "/home/zzm/sti-data"
    caption_coco = COCO(path_caption)
    ...
    # field
    path_label="/home/zzm/data-new/label-end/field/results1/"
    path_draw="/home/zzm/data-new/draw-end/field/results1/"
    all_file=getAllFiles(path_label)

    for sjson in tqdm(all_file):
        sketch = Sketch(label_end_path=os.path.join(path_label, sjson),
                        draw_end_path=os.path.join(path_draw, sjson),
                        image_path=path_image,
                        caption_coco=caption_coco, useless=True, color=False)
        name = sketch.image_name.split('.')[0]
        train1.append(name + '.json')

    # sport
    path_label = "/home/zzm/data-new/label-end/sport/results1/"
    path_draw = "/home/zzm/data-new/draw-end/sport/results1/"
    all_file = getAllFiles(path_label)
    for sjson in tqdm(all_file):
        sketch = Sketch(label_end_path=os.path.join(path_label, sjson),
                        draw_end_path=os.path.join(path_draw, sjson),
                        ...
                        caption_coco=caption_coco, useless=True, color=False)
        name = sketch.image_name.split('.')[0]
        train2.append(name + '.json')

    # vehicle
    path_label = "/home/zzm/data-new/label-end/vehicle/results1/"
    path_draw = "/home/zzm/data-new/draw-end/vehicle/results1/"
    all_file = getAllFiles(path_label)
    for sjson in tqdm(all_file):
        sketch = Sketch(label_end_path=os.path.join(path_label, sjson),
                        draw_end_path=os.path.join(path_draw, sjson),
                        ...
                        caption_coco=caption_coco, useless=True, color=False)
        name = sketch.image_name.split('.')[0]
        train3.append(name + '.json')


    test1 = random.sample(train1, 1075)
    test2 = random.sample(train2, 1155)
    test3 = random.sample(train3, 770)
    all=train1+train2+train3
    ...
    for temp in all:
        if temp not in test:
            train.append(temp)
    for temp in train:
        copyfile(os.path.join(save_path,'sketch',temp),os.path.join(save_path,'train',temp))
    for temp in test:
        copyfile(os.path.join(save_path,'sketch',temp),os.path.join(save_path,'test',temp))

def save_model_json_image():
    path_image = "/home/zzm/datasets/coco2017/train2017"
    path_caption = "/home/zzm/datasets/coco2017/annotations/captions_train2017.json"
    ...
    all_file=getAllFiles(path_label)
    for sjson in tqdm(all_file):
        sketch= Sketch(label_end_path=os.path.join(path_label,sjson),draw_end_path=os.path.join(path_draw,sjson),image_path=path_image,
                caption_coco=caption_coco,useless=True,color=False,scene="field")
        img=sketch.get_image()
        name=sketch.image_name.split('.')[0]
        sketch.save_json(os.path.join(save_path,'sketch',name+'.json'))
        Sketch.save_image(img,os.path.join(save_path,'sketch-png',name+'.png'))
        copyfile(os.path.join(path_image,sketch.image_name),os.path.join(save_path,'image',sketch.image_name))
    ...
    for sjson in tqdm(all_file):
        try:
            sketch = Sketch(label_end_path=os.path.join(path_label, sjson), draw_end_path=os.path.join(path_draw, sjson),
                            image_path=path_image,
                            caption_coco=caption_coco, useless=True, color=False,scene="sport")
            img = sketch.get_image()
            name = sketch.image_name.split('.')[0]
            sketch.save_json(os.path.join(save_path, 'sketch', name + '.json'))
            Sketch.save_image(img, os.path.join(save_path, 'sketch-png', name + '.png'))
    ...
    for sjson in tqdm(all_file):
        try:
            sketch = Sketch(label_end_path=os.path.join(path_label, sjson),
                            draw_end_path=os.path.join(path_draw, sjson),
                            image_path=path_image,
                            caption_coco=caption_coco, useless=True, color=False,scene="vehicle")
            img = sketch.get_image()
            name = sketch.image_name.split('.')[0]
            sketch.save_json(os.path.join(save_path, 'sketch', name + '.json'))
            Sketch.save_image(img, os.path.join(save_path, 'sketch-png', name + '.png'))
...
def main():
    ...
    sketch = Sketch(label_end_path="/home/zzm/data-new/label-end/field/results1/100.json",image_path="/home/zzm/datasets/coco2017/train2017"
                    ,draw_end_path="/home/zzm/data-new/draw-end/field/results1/100.json",caption_coco='/home/zzm/datasets/coco2017/annotations/captions_train2017.json',useless=True)
